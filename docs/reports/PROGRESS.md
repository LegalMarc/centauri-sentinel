...
- Notes: Three-service Docker Compose stack (token-init → obico-ml → sentinel). GHCR CI workflow for obico-ml (build-obico-ml.yml). Deployment via Coolify using the /api/v1/applications/public endpoint with build_pack=dockercompose (the /dockercompose endpoint is absent in this Coolify build). Three fixes required during deployment: (1) obico-ml source copy path and CMD fixed in PR #43; (2) 5 GB CUDA base image replaced with python:3.10-slim + onnxruntime CPU in PR #44 — the deploy host has no NVIDIA GPU and limited free disk; (3) Coolify env vars set: PRINTER_IP, PRINTER_ACCESS_CODE, EXTERNAL_BIND_ALLOWED=true, SENTINEL_PORT=18000. Sentinel live with SSL via Traefik at the configured domain, /healthz → {"status":"ok"}. Both sentinel and obico-ml containers running:healthy.
