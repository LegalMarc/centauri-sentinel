...
def validate_printer_ip(v: str) -> str:
    """
    Validates a printer IP or hostname, enforcing SSRF protections
    against link-local, multicast, and cloud metadata IPs.
    """
    v_str = v.strip()
    if not v_str:
...
def validate_https(url: str) -> str:
    ...
    if url:
        ...
        if parsed.hostname and parsed.scheme == "http":
            if parsed.hostname in ("localhost", "127.0.0.1") or "." not in parsed.hostname:
                return url
            with contextlib.suppress(ValueError):
